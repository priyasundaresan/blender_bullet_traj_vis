...
def keyframe_traj(context, files, directory, mode='urdf'):
    assert(mode == 'urdf' or mode == 'waypoints')
    if mode == 'waypoints':
...
#def load_pybullet_pkl(context, files, directory):
#    for file in files:
#        filepath = join(directory, file)
#        print(f'Processing {filepath}')
#        with open(filepath, 'rb') as pickle_file:
...
def main():
    ...
    #frame_count = keyframe_traj(bpy.context, files=['recorder.pkl'], directory=os.path.abspath(os.getcwd()), mode='waypoints')
    frame_count = keyframe_traj(bpy.context, files=['recorder.pkl'], directory=os.path.abspath(os.getcwd()), mode='urdf')
    output_dir = 'images'
